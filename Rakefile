...
# --- Upgrade safety nets -------------------------------------------------
#
# The build is generated by govuk_tech_docs and middleman, so an upgrade to
# either can change how pages are discovered, how frontmatter is read, or
# which files get rendered - and the symptom is pages quietly not appearing
# rather than an error. html-proofer catches broken links in what was built;
# it cannot notice that a page is missing altogether.
#
# These two tasks close that gap, so a dependency bump either passes or says
# what it broke.

# Every page tech-docs renders needs these, and the sidebar ordering and the
# review banner are built from them. A page missing `weight` lands in an
# arbitrary place in the nav; one missing `last_reviewed_on` renders without
